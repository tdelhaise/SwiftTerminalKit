...
public final class Surface {
	...
	@inline(__always) private func idx(_ x: Int, _ y: Int) -> Int { y * width + x }
	...
	@inline(__always) private func paletteEqual(_ a: Console.PaletteColor, _ b: Console.PaletteColor) -> Bool {
		switch (a, b) {
			case (.named(let n1), .named(let n2)): return n1 == n2
			case (.gray(let g1), .gray(let g2)):   return g1 == g2
	...
	public func resize(_ w: Int, _ h: Int) {
		...
		for y in 0..<ch { for x in 0..<cw { next[y * nw + x] = cur[y * width + x] } }
	...
	public func putString(x: Int, y: Int, text: String, fg: Console.PaletteColor = .default, bg: Console.PaletteColor = .default) {
		guard y >= 0, y < height, x < width else { return }
		var cx = max(0, x)
		for g in text {
			if cx >= width { break }
			put(cx, y, Cell(String(g), fg: fg, bg: bg)); cx += 1
		}
	}
		...
		for r in regions {
			...
			for y in y0..<y1 {
				...
				while x < x1 {
					...
					while x < x1 && cellEqual(cur[idx(x,y)], prev[idx(x,y)]) { x += 1 }
					if x >= x1 { break }
					
					let start = x
					let first = cur[idx(x,y)]
					var runFG = first.fg, runBG = first.bg
					x += 1
					while x < x1 {
						let c = cur[idx(x,y)], p = prev[idx(x,y)]
						if cellEqual(c, p) { break }
						if !paletteEqual(c.fg, runFG) || !paletteEqual(c.bg, runBG) { break }
						x += 1
					}
					...
					for xx in start..<x { s.append(cur[idx(xx,y)].ch) }
					console.write(s)
				}
			}
		}
		if prev.count != cur.count { prev = Array(repeating: Cell(), count: cur.count) }
		for i in 0..<cur.count { prev[i] = cur[i] }
